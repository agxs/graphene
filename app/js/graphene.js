...
Graphene.GaugeLabelView = (function(_super) {
  ...
  GaugeLabelView.prototype.events = {
    "click #minus-time": "minusTime",
    "click #plus-time": "plusTime"
  };

  GaugeLabelView.prototype.minusTime = function() {
    return _displaceTime( this, -30 );
  };

  GaugeLabelView.prototype.plusTime = function() {
    return _displaceTime( this, 30 );
  };
...
Graphene.TimeSeriesView = (function(_super) {
  ...
  TimeSeriesView.prototype.render = function() {
    var area, circlePoints, d, data, dmax, dmin, leg_items, line, litem_enters, litem_enters_text, order, points, title, vis, x, xAxis, xmax, xmin, xpoints, xtick_sz, y, yAxis, _ref,
      _this = this;
    console.log("rendering.");
    data = this.model.get('data');
    ...
    xAxis = d3.svg.axis().scale(x).ticks(4).tickSize(xtick_sz).tickFormat(d3.time.format("%H:%M")).tickSubdivide(true);
    ...
    _.each(points, function(p, i) {
      ...
      return circlePoints = circlePoints.concat(p);
    });
    this.circleTooltips;
    if (this.firstrun) {
      this.firstrun = false;
      ...
      this.circleTooltips = vis.selectAll('circle').data(circlePoints).enter().append('circle').attr('r', 5).attr('cx', function(d) {
        return x(d[1]);
      }).attr('cy', function(d) {
        return y(d[0]);
      ...
      this.circleTooltips.text(function(d) {
        return d[0];
      });
      if (this.title) {
    ...
    vis.selectAll("path.line").data(points).attr("d", line).transition().ease("linear").duration(this.animate_ms);
    vis.selectAll('circle').data(circlePoints).attr('cx', function(d) {
      return x(d[1]);
    }).attr('cy', function(d) {
      return y(d[0]);
    }).transition().ease('linear').duration(this.animate_ms);
    this.circleTooltips.remove();
    this.circleTooltips = vis.selectAll('circle').data(circlePoints).append('title');
    return this.circleTooltips.text(function(d) {
      return d[0];
    });
  };

  TimeSeriesView.prototype.events = {
    "click #minus-time": "minusTime",
    "click #plus-time": "plusTime"
  };
...
function _displaceTime( view, factor ) {
  var source = view.model.get('source');
  var from = parseInt( source.match( /.*from=(-\d+)min&/ )[1] );
  var newFrom = from + factor;
  newFrom = newFrom > -60 ? -60 : newFrom;

  var until = source.match( /.*until=(now.*?)&/ )[1];
  var newUntil = '';
  var untilTime = '';

  if ( until === 'now' ) {
    newUntil = factor < 0 ? 'now' + factor + 'min' : 'now';
  }
  else {
    untilTime = parseInt( until.match( /now(-\d+)min/ )[1] );
    untilTime += factor;

    newUntil = untilTime < 0 ? 'now' + untilTime + 'min' : 'now';
  }

  source = source.replace( 'from=' + from, 'from=' + newFrom );
  source = source.replace( 'until=' + until, 'until=' + newUntil );

  view.model.set({
    source: source
  });
  return view.model.refresh();
}
